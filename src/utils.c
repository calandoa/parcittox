...
struct cmdline_opts *parse_options(int argc, char* argv[])
{
	...
	GOptionEntry main_entries[] = {
		...
		},
		{
			...
			0,
			G_OPTION_ARG_NONE,
			&opts->icon, _("Do not use status icon (Super-P for menu)"),
			NULL
		},
		{
			"clipboard", 'c',
		...
		},
		{
			...
			0,
			G_OPTION_ARG_NONE,
			&opts->primary, _("Print selection contents"),
			NULL
		},
		{
			"pref", 'p',
			0,
			G_OPTION_ARG_NONE,
			&opts->pref, _("Open Preferences dialog"),
			NULL
		},
		{
			NULL
	...
	g_option_context_set_summary(context,
				     _("Clipboard CLI usage examples:\n\n"
				       "  echo \"copied to clipboard\" | " PARCELLITE_PROG_NAME "\n"
				       "  " PARCELLITE_PROG_NAME " \"copied to clipboard\"\n"
				       "  echo \"copied to clipboard\" | " PARCELLITE_PROG_NAME " -c"));
	/* Set description */
...
/*	g_printf("My PID is %d\n",getpid()); */
	...
	if(mode&PROG_MODE_DAEMON){
		...
		if(f->dbg) g_printf("running " PARCELLITE_PROG_NAME " not found\n");
		if(create_fifo() < 0 ){
		...
		if(f->dbg) g_printf(PARCELLITE_PROG_NAME " found\n");
		if(open_fifos(f) < 0 ){
			g_printf("Error opening fifo. Exit\n");
			goto err;
		}
