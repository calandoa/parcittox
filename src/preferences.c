...
#define DEF_PHISTORY_KEY	"<Super>B"
#define DEF_HISTORY_KEY		"<Super>V"
#define DEF_ACTIONS_KEY		"<Super>A"
#define DEF_MENU_KEY		"<Super>P"
...
#define PREF_SEC_DTX_COMMON	6
#define PREF_SEC_DTX_TX		7
#define PREF_SEC_DTX_RX		8
...
struct myadj align_dtx_retry = {0,0x7FFFFFFF,1,100};
...
/**Behaviour	*/
	...
	/**History	*/
	{.adj=NULL,.cval=NULL,.sig=NULL,.sec=PREF_SEC_HIST,.name="save_history",.type=PREF_TYPE_TOGGLE,.desc="Save",.tip="Save history to a file.",.val=DEF_SAVE_HISTORY},
	{.adj=NULL,.cval=NULL,.sig=NULL,.sec=PREF_SEC_HIST,.name="history_pos",.type=PREF_TYPE_TOGGLE|PREF_TYPE_SINGLE_LINE,.desc="Position",.tip="If checked, use X, Y to position the history popup",.val=0},
	...
	{.adj=&align_hist_lim,.cval=NULL,.sig=NULL,.sec=PREF_SEC_HIST,.name="history_limit",.type=PREF_TYPE_SPIN,.desc="Max items count",.tip="Maximun count of cliboard entries to keep",.val=DEF_HISTORY_LIMIT},
...
/**Action Keys	*/
#ifdef HAVE_DITTOX
	{.adj=NULL,.cval=NULL,.sig=NULL,.sec=PREF_SEC_DTX_COMMON,.name="dittox_pwd",.type=PREF_TYPE_ENTRY|PREF_TYPE_EXPEND,.desc="Password: ",.tip="Must be the same for every host"},
	{.adj=NULL,.cval=NULL,.sig=NULL,.sec=PREF_SEC_DTX_COMMON,.name="dittox_dialog",.type=PREF_TYPE_TOGGLE,.desc="Show error dialogs",.tip="If checked, an annoying dialog window will popup for each new error (usefull for debug)."},
	{.adj=NULL,.cval=NULL,.sig=NULL,.sec=PREF_SEC_DTX_TX,.name="dittox_hosts",.type=PREF_TYPE_ENTRY|PREF_TYPE_EXPEND,.desc="Hosts:      ",.tip="IPv4, IPv6 or hostname, separated by commas, to which send local clips."},
	{.adj=&align_dtx_retry,.cval=NULL,.sig=NULL,.sec=PREF_SEC_DTX_TX,.name="dittox_retry",.type=PREF_TYPE_SPIN,.desc="Delay before retry after error",.tip="Delay in seconds. 0 will discard the host permanently in case of errors.",.val=0},
	{.adj=NULL,.cval=NULL,.sig=NULL,.sec=PREF_SEC_DTX_TX,.name="dittox_manual",.type=PREF_TYPE_TOGGLE,.desc="Disable automatic sending",.tip="If checked, clips will only be sent if user orders it from submenu."},
	{.adj=NULL,.cval=NULL,.sig=NULL,.sec=PREF_SEC_DTX_RX,.name="dittox_ignore",.type=PREF_TYPE_TOGGLE,.desc="Do not listen to other hosts",.tip="If checked, history will not be updated with clips from other hosts."},
#endif /* HAVE_DITTOX */
...
int add_section(int sec, GtkWidget *parent)
{
	...
	for (i = get_first_pref(sec);sec==myprefs[i].sec; ++i){
		connect=1;
		single_st=(myprefs[i].type & PREF_TYPE_SINGLE_LINE); /**deterimine if we are in single line	*/
		...
		switch (myprefs[i].type&PREF_TYPE_MASK){
			...
			case PREF_TYPE_SPIN:
				...
				g_assert(myprefs[i].adj);
				myprefs[i].w = gtk_spin_button_new((GtkAdjustment*)
					gtk_adjustment_new(myprefs[i].val, myprefs[i].adj->lower, myprefs[i].adj->upper, myprefs[i].adj->step, myprefs[i].adj->page, 0),
					10, 0);
			...
			case PREF_TYPE_ENTRY:
				fixed = !(myprefs[i].type & PREF_TYPE_EXPEND);
				if (!single_is)
					hbox = gtk_hbox_new(fixed, 4);
				label = gtk_label_new(NULL);
				gtk_label_set_markup((GtkLabel*)label, _(myprefs[i].desc));
				gtk_misc_set_alignment((GtkMisc*)label, 0.0, 0.50);
				gtk_box_pack_start((GtkBox*)hbox, label, fixed, TRUE, 0);
		...
		single_st = myprefs[i+1].type & PREF_TYPE_SINGLE_LINE; /**deterimine if we are in single line	*/
...
/**miscelleaneous	*/
...
#if HAVE_DITTOX
	/* Build the Network page (dittox) */
	GtkWidget* page_network = gtk_alignment_new(0.50, 0.50, 1.0, 1.0);
	gtk_alignment_set_padding((GtkAlignment*)page_network, 12, 6, 12, 6);
	gtk_notebook_append_page((GtkNotebook*)notebook, page_network, gtk_label_new(_("Network")));
	GtkWidget* vbox_network = gtk_vbox_new(FALSE, 12);
	gtk_container_add((GtkContainer*)page_network, vbox_network);

	/* Build the Common Network frame (dittox) */
	frame = gtk_frame_new(NULL);
	gtk_frame_set_shadow_type((GtkFrame*)frame, GTK_SHADOW_NONE);
	label = gtk_label_new(NULL);
	gtk_label_set_markup((GtkLabel*)label, _("<b>Common</b>"));
	gtk_frame_set_label_widget((GtkFrame*)frame, label);
	alignment = gtk_alignment_new(0.50, 0.50, 1.0, 1.0);
	gtk_alignment_set_padding((GtkAlignment*)alignment, 12, 0, 12, 0);
	gtk_container_add((GtkContainer*)frame, alignment);
	...
	label = gtk_label_new(_(
		"Integration of the Dittox library enables to share the content of the clipboard with several hosts\n\n"
		"These hosts must be configured with the same settings. The TCP listen port is " STR(DITTOX_LISTEN_PORT)
		", and communications are encrypted with a password and compatible with Ditto for Windows.\n"));
	gtk_label_set_line_wrap((GtkLabel*)label, TRUE);
	gtk_box_pack_start((GtkBox*)vbox, label, TRUE, TRUE, 0);
	add_section(PREF_SEC_DTX_COMMON, vbox);
	gtk_box_pack_start((GtkBox*)vbox_network,frame,FALSE,FALSE,0);

	/* Build the Tx Network frame (dittox) */
	frame = gtk_frame_new(NULL);
	gtk_frame_set_shadow_type((GtkFrame*)frame, GTK_SHADOW_NONE);
	label = gtk_label_new(NULL);
	gtk_label_set_markup((GtkLabel*)label, _("<b>Transmission</b>"));
	gtk_frame_set_label_widget((GtkFrame*)frame, label);
	alignment = gtk_alignment_new(0.50, 0.50, 1.0, 1.0);
	gtk_alignment_set_padding((GtkAlignment*)alignment, 12, 0, 12, 0);
	gtk_container_add((GtkContainer*)frame, alignment);
	...
	vbox = gtk_vbox_new(FALSE, 2);
	gtk_container_add((GtkContainer*)alignment, vbox);
	add_section(PREF_SEC_DTX_TX, vbox);
	gtk_box_pack_start((GtkBox*)vbox_network,frame,FALSE,FALSE,0);

	/* Build the Rx Network frame (dittox) */
	frame = gtk_frame_new(NULL);
	gtk_frame_set_shadow_type((GtkFrame*)frame, GTK_SHADOW_NONE);
	label = gtk_label_new(NULL);
	gtk_label_set_markup((GtkLabel*)label, _("<b>Reception</b>"));
	gtk_frame_set_label_widget((GtkFrame*)frame, label);
	alignment = gtk_alignment_new(0.50, 0.50, 1.0, 1.0);
	gtk_alignment_set_padding((GtkAlignment*)alignment, 12, 0, 12, 0);
	gtk_container_add((GtkContainer*)frame, alignment);
	...
	vbox = gtk_vbox_new(FALSE, 2);
	gtk_container_add((GtkContainer*)alignment, vbox);
	add_section(PREF_SEC_DTX_RX, vbox);
	gtk_box_pack_start((GtkBox*)vbox_network,frame,FALSE,FALSE,0);
